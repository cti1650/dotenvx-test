...
⚠️ **注意**: `.git/hooks/` はGit管理外のため、**他の開発者には共有されません**。チームで使う場合は次のHuskyを利用してください。

### lefthookでチーム全体に共有する

`lefthook.yml`をGit管理することで、`npm install`時に全員の環境で自動セットアップされます。Goで書かれた単一バイナリで動作が速く、複数チェックを並列実行できるのが特徴です。
