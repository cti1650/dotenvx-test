...
ただし `dotenvx ext precommit` は平文混入だけでなく**`.env.keys`の.gitignore登録漏れも検知**し、修正コマンドまで提示します。「ユーザーは間違えるもの」を前提にツール側で対応されているため、**lefthookには `dotenvx ext precommit` を呼ぶだけで十分**です。
...
[dotenvx](https://dotenvx.com/)は、dotenvの作者による次世代の環境変数管理ツールです。各機能の詳細は[公式ドキュメント](https://dotenvx.com/docs)を参照してください。
...
冒頭の設計思想を未読でも安全に運用するための最小チェック項目です。

- [ ] `.env.keys` を `.gitignore` に登録し、コミットしない
- [ ] 本番環境の秘密鍵はCI/CDの環境変数（GitHub Secrets等）で管理
- [ ] コミット前の平文混入チェックを自動化（後述の「コミット前の自動チェック」を参照）

## コミット前の自動チェック

平文の`.env`の混入や`.env.keys`のコミット事故を防ぐため、dotenvx標準のpre-commitチェック機能を利用できます。
...
> 大規模運用・監査要件がある組織は dotenvx の範囲外（[このリポジトリの位置付け](#このリポジトリの位置付け)を参照）

## このリポジトリの位置付け

dotenvxは利便性とチーム開発の効率を高める一方、**`.env.keys`の管理が単一の重要点**になり、鍵管理コストはむしろ上がります。これはトレードオフであり、銀の弾丸ではありません。

運用コストを抑えつつ煩雑な`.env`管理を改善したい層には適切な選択肢になりうる一方、組織規模・運用成熟度・監査要件によっては別のアプローチが適している場合もあります。本リポジトリは、その判断を実プロジェクト導入前に行うための最小構成サンプルとして公開しています。
